...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!userId) return;
      try {
        const res = await axios.get(`http://api.biziffy.com/api/auth/get-user-by-id/${userId}`);
    ...
    const fetchBussinessListing = async () => {
      if (!userId) return;
      try {
        const res = await axios.get(`http://api.biziffy.com/api/get-all-listings-by-user-id/${userId}`);
    ...
    const fetchWebsiteListing = async () => {
      if (!userId) return;
      try {
        const res = await axios.get(`http://api.biziffy.com/api/admin/get-all-website-listings-by-user-id/${userId}`);
  ...
  //   try {
  //     const response = axios.delete(`http://api.biziffy.com/api/delete-business-listing/${id}`);
  ...
  const handleSaveChanges = async () => {
    try {
      if (profileData && profileData._id) {
        const response = await axios.post(`http://api.biziffy.com/api/auth/update-user/${profileData._id}`, profileData);
  ...
  const uploadProfileImage = async () => {
    ...
    try {
      const res = await axios.post(`http://api.biziffy.com/api/auth/upload-profile-image/${profileData._id}`, formData);
