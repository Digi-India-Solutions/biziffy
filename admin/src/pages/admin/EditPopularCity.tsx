...
const EditPopularCity = () => {
  ...
  useEffect(() => {
    const fetchCityDetails = async () => {
      ...
      try {
        setLoading(true);
        const res = await axios.get(`http://localhost:18001/api/populerCity/get-popular-city-by-id/${id}`);
    ...
    const fetchInitialData = async () => {
      try {
        const [cityRes, catRes] = await Promise.all([
          axios.get("http://localhost:18001/api/city/get-all-city"),
          axios.get("http://localhost:18001/api/categories"),
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post(`http://localhost:18001/api/populerCity/update-popular-city/${id}`, form);
      if (res?.data?.status) {
