...
export const AllListings = () => {
  ...
  const fetchFullListings = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`https://api.biziffy.comapi/get-all-listings`);
  ...
  const handleBulkAction = async () => {
    ...
    try {
      await axios.post(`https://api.biziffy.comapi/listing-bulk-action`, {
  ...
  const handleUpdatePublishStatus = async (id: string, newStatus: string) => {
    try {
      await axios.post(`https://api.biziffy.comapi/change-publish-status/${id}`, { status: newStatus });
      setFullListings(
  ...
  const handleDeleteListing = async (id: string) => {
    ...
    try {
      const response = await axios.get(`https://api.biziffy.comapi/delete-business-listing/${id}`);
