...
const SupportTickets = () => {
  ...
  const fetchSupportTickets = async () => {
    try {
      const res = await axios.get("http://localhost:18001/api/admin/support-tickets");
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:18001/api/admin/support-tickets/${selectedTicket._id}`);
  ...
  const handleStatusChange = async (newStatus: string) => {
    if (!selectedTicket) return;
    try {
      await axios.put(`http://localhost:18001/api/admin/support-tickets/${selectedTicket._id}`, {
        status: newStatus,
      });
