...
//       try {
//         const res = await axios.get("http://localhost:18001/api/contactus/get-all-contact");
...
const ContactUs = () => {
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    try {
      await axios.post(`http://localhost:18001/api/contactus/update-status/${id}`, { status: newStatus, });
      setContactData((prev) =>
        prev.map((contact) =>
