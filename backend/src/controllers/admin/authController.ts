  ...
  try {
    ...
    const results = await Promise.all(
      userIds.map(async (id) => {
        ...
        if (user) {
          user.status = status as "Active" | "Inactive" | "pending";
          await user.save();
...
export const sendOtpHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    (user as any).otp = otp;
    (user as any).otpExpiry = new Date(Date.now() + 10 * 60 * 1000); // Optional: 10 min expiry
    await user.save();
...
export const verifyOtpHandler = async (req: Request, res: Response) => {
  ...
  try {
    const user = await User.findOne({ email });
    if (!user || (user as any).otp !== otp)
      return res.status(400).json({ message: "Invalid OTP" });

    if ((user as any).otpExpiry && (user as any).otpExpiry < new Date())
